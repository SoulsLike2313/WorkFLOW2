...
3. basic signature assurance gate semantics;
4. federation command terminology split (sovereign vs operational vs delegated).
...
3. no department-specific claim override model (intentionally out of scope).
...
1. per-department escalation semantics are governance-documented but not fully machine-validated per mission/program run.

Draft/inferred only:
1. per-department permanent oversight ownership;
2. exception policy for department-specific sovereign-signoff shortcuts.
